...
const Home = (props: props) => {
	...
	return props?.data?.map((anime: anime) => {
		return (
			<div className="flex mt-4" key={anime?.mal_id}>
				<img src={anime?.images?.jpg?.image_url} alt="애니" className="mr-4" />
				<div className="flex flex-col py-4 overflow-hidden justify-between">
					...
					<div className="grid grid-cols-2">
						...
						<div className="flex items-center">
							<span>방영 기간 :&nbsp;</span>
							<span className="font-bold text-lg">{anime?.aired?.string}</span>
						</div>
						<div className="flex items-center">
							<span>방영 매체 :&nbsp;</span>
					...
					<Link href={anime?.url} replace>
						<a className="w-fit text-white ml-auto bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
							상세보기
						</a>
					</Link>
				</div>
...
export const getStaticProps: GetStaticProps = async () => {
	const res = await axios.get("https://api.jikan.moe/v4/anime/", {
		params: { page: 1, limit: 10, sfw: false, order_by: "score", sort: "desc" },
	});
